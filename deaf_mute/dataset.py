...
class RecursiveCaptionDataset(Dataset):
    ...
    """
    def __init__(self, data_dir, video_feature_dir, duration_file, word2idx_path,
                 max_t_len, max_v_len, max_n_sen, mode="train", recurrent=True, untied=False):
        """
        list of files requiered :
            - word2idx_path
            - data_dir # containing training data (train.json, val.json, test.json)
            - video_feature_dir # cur_path_resnet (video_feature_dir/*.npy) *=video name
            - duration_file : csv containing the video file name, they duration and number of frame per video
        """
    ...
    def set_data_mode(self, mode):
        ...
        if mode == "train": 
            ...
            data_path = os.path.join(self.data_dir, "val.json")
        elif mode == "test":  
            data_path = os.path.join(self.data_dir, "test.json")
        else:
    ...
    def fix_missing(self):
        ...
        for e in tqdm(self.data):
            video_name = e["name"]
            cur_path = os.path.join(self.video_feature_dir, "{}.npy".format(video_name))
    ...
    def clip_sentence_to_feature_untied(self, name, timestamp, sentence, raw_video_feature):
        ...
        """
        frm2sec = self.frame_to_second[name]
